...
def setup_environment():
    ...
    if not all_packages_installed:
        print(f"{Fore.YELLOW}Installing required packages: {', '.join(REQUIRED_PACKAGES)}...{Style.RESET_ALL}")
        try:
        ...
        print(f"{Fore.YELLOW}All required packages are already installed.{Style.RESET_ALL}")

    print(f"{Fore.BLUE}Relaunching script in virtual environment...{Style.RESET_ALL}")
    try:
        os.execv(python_executable, [python_executable, sys.argv[0]] + sys.argv[1:])
    except Exception as e:
...
def display_help():
    ...
    print("      - " + Fore.CYAN + "Image Key:" + Fore.RESET + " Uses another image as the key. Each pixel of the input image is XORed with the corresponding pixel of the key image. The key image will be automatically resized to match the input image. You must use the EXACT SAME key image for decryption.")
...
def main_program_loop():
    ...
    while True:
        if current_step == 'MAIN_MENU':
            ...
            if mode_input == 'QUIT':
                print(Fore.CYAN + "Exiting Image Encryption Tool. Goodbye!")
                sys.exit()
